defmodule ExDTLS.IntegrationTest do
  ...
  test "dtls_srtp" do
    ...
    {packets, _timeout, tx_dtls} = ExDTLS.do_handshake(tx_dtls)
  ...
  defp loop({dtls1, state1}, {dtls2, state2}, packets) do
    case ExDTLS.process(dtls1, packets) do
      {:handshake_packets, packets, _timeout, dtls1} ->
        loop({dtls2, state2}, {dtls1, state1}, packets)

      {:handshake_finished, _lkm, _rkm, _p, packets, dtls1} ->
        loop({dtls2, state2}, {dtls1, true}, packets)

      {:handshake_finished, _lkm, _rkm, _p, dtls1} ->
        loop({dtls2, state2}, {dtls1, true}, packets)
    end
  end
end
